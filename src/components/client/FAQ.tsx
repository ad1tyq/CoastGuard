...
function FAQ() {
    ...
    return (
        ...
        <h1 className="text-center text-xl font-bold md:text-4xl mb-10 py-3 w-[40rem]">Frequently Asked Questions</h1>
            ...
            {faqData.map((faq) => {
                return (
                    <div key={faq.id} onMouseEnter={() => setOpen(faq.id)} onMouseLeave={()=>setOpen(null)}
                        className={`flex flex-col items-center hover:bg-black/30 bg-gray-200 w-[60rem] gap-2 py-5 rounded-[3rem]
                        `}>
